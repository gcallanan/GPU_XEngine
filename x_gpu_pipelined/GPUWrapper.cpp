...
void GPUWrapper::operator()(boost::shared_ptr<PipelinePacket> inPacket, multi_node::output_ports_type &op){
    ...
    while(inPacketQueue->getArmortiserSize() > 0)
    {
        ...
        if(numAccumulations == accumulationsThreshold-1){
            ...
            if(!std::get<0>(op).try_put(boost::dynamic_pointer_cast<PipelinePacket>(tempGpuWrapperPacket))){
